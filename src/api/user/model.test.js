...
beforeEach(async () => {
  user = await User.create({ login: 'user', email: 'a@a.com', password: '123456' })
})

describe('set email', () => {
  it('sets login automatically', () => {
    user.login = ''
...
describe('view', () => {
  it('returns simple view', () => {
    ...
    expect(view.login).toBe(user.login)
  ...
  ;['facebook', 'github', 'google'].forEach((service) => {
    describe(service, () => {
      ...
      it('updates user when email is already registered', async () => {
        ...
        // update
        expect(updatedUser.login).toBe(serviceUser.login)
      ...
      it('creates a new user when neither service id and email was found', async () => {
        ...
        expect(createdUser.login).toBe(serviceUser.login)
