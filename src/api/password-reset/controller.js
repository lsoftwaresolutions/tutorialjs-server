...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your TutorialJS account.<br>
        ...
        &mdash; TutorialJS Team
      `
      return sendMail({ toEmail: email, subject: 'TutorialJS - Password Reset', content })
    })
